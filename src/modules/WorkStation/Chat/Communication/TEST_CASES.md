...
2. "Open in Workstation" action (ArrowUpRight) opening the plan file.
...
6. **Edits survive re-view**: after Save, re-opening the preview / chat card /
   the file via "Open in Workstation" shows the edited content. — fixed bug
...
**file** still holds the edit (Build + Open in Workstation stay correct). Closing
...
| --- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Open Communication while a plan is pending.               | Preview auto-opens; trailing row shows Source/Preview pill + **Edit** + **Open in Workstation** (ArrowUpRight).                                                                |
| 2   | Click **Edit**.                                           | View switches to preview; plan renders as an editable textarea (`plan-doc-editor`); trailing row now shows **only Cancel + Save** (pill, Edit, Open in Workstation hidden).    |
| 3   | Modify the text, click **Save**.                          | Edits persist (plan file + plan event + snapshot); edit mode exits back to Edit + Open in Workstation; the preview now shows the **edited** content. **No build/turn starts.** |
| 4   | After Save, re-open the preview (or switch tab and back). | The edited content is still shown (re-view reflects the edit).                                                                                                                |
| 5   | After Save, click **Open in Workstation**.                 | The plan `.md` opens in Workstation Code dock showing the **edited** content.                                                                                                  |
| 6   | After Save, Build the plan (chat card **Build**).         | Backend reads the saved plan file and executes the **edited** plan.                                                                                                           |
| 7   | Enter edit, click **Cancel**.                             | Edit discarded; textarea reverts to preview; trailing row returns to Edit + Open in Workstation.                                                                               |
...
| --- | ------------------------------------------ | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 1   | No plan path resolved                      | Pending plan whose `planPath` is empty.           | "Open in Workstation" hidden; Save still patches the plan event + snapshot (no file write); editing still works.                    |
...
- [ ] Edits are visible on re-view (preview, chat card, Open in Workstation file).
- [ ] "Open in Workstation" uses ArrowUpRight and opens the plan file via `openFileInWorkStation`.
