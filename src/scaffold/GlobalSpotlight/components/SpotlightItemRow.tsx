...
export const ITEM_HEIGHT_WITH_DESC = 50;
...
const DescLine = memo<{ desc: string; descTitle: unknown }>(
  ({ desc, descTitle }) => {
    if (typeof descTitle !== "string" || !descTitle) {
      return <div className="truncate text-[11px] text-text-2">{desc}</div>;
    }
    ...
    const tooltipContent = (
      <div className="flex flex-col gap-0.5">
        {descTitle.split(", ").map((name) => (
          <span key={name} className="whitespace-nowrap text-[11px]">
            {name}
          </span>
        ))}
      </div>
  ...
  ({
    ...
    return (
      <div
        ...
        className={`spotlight-item group mx-2 mb-[4px] flex items-center gap-3 rounded-lg px-2 ${
          isDisabled
      ...
      >
        ...
        {isChildItem && (
          <div className="flex w-5 flex-shrink-0 items-center justify-center">
            <CornerDownRight className="text-text-2" size={12} />
          </div>
        )}
        ...
        {item.icon && (
          <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg bg-fill-2">
            {isCurrentSelection ? (
              ...
              <i className={`${item.icon} text-[18px] ${itemIconClassName}`} />
            ) : (
        ...
        <div className="min-w-0 flex-1 basis-0">
          <div className="flex min-w-0 items-center gap-2">
            {item.type === "hint" && data.prefix ? (
              ...
              <span className="flex min-w-0 items-center gap-1.5 truncate text-[14px]">
