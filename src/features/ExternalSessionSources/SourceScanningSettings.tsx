 ...
 *
 * Inventory of every external coding tool ORGII detects, driven
 ...
 * `useSourceScanningInventory`; the settings-table column definitions live in
 * `buildSourceScanningColumns`. This file wires that state to the tab/search
 * UI and the table/expandable-row layout.
 *
 * Title-less on purpose: Settings → Import and Runtime → Scanning both render
...
import { buildSourceScanningColumns } from "./sourceScanningColumns";
import type { DataSourceTab, SourceRow } from "./sourceScanningTypes";
import { useSourceScanningInventory } from "./useSourceScanningInventory";

/** Product term, not prose: the CLI filter reads "CLI" in every locale. */
const CLI_TAB_LABEL = "CLI";

const SourceScanningSettings: React.FC = () => {
          ...
          rightContent:
            (rows?.length ?? 0) > 0 ? (
              <RefreshButton
                ...
                disabled={!externalSessionsEnabled}
                dataTestId="source-scanning-rescan-all"
              />
            ) : undefined,
          tabPills: (
            <TabPill
...
export default SourceScanningSettings;
