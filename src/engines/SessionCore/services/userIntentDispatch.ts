...
interface PrepareUserIntentParams {
  ...
  /** Reuse an existing optimistic projection owned by this exact Session. */
  userEventId?: string;
...
export async function dispatchUserIntent(
  ...
  try {
    await deliverOptimisticOutgoing({
      send: () => {
        const phase = getTurnPhase(preparation.sessionId);
        if (
          getTurnGeneration(preparation.sessionId) !== preparation.generation ||
          (phase !== "dispatching" && phase !== "working")
        ) {
          throw new Error(
            "User intent was stopped or superseded before dispatch"
          );
        }
