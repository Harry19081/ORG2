...
describe("processChatItems", () => {
  ...
  describe("collapseInterToolNarration", () => {
    const makeAssistantMsg = (id: string, text: string) =>
      makeSessionEvent({
        id,
        action_type: "assistant",
        function: "assistant_message",
    ...
    const makeUserMsg = (id: string, text: string) =>
      makeSessionEvent({
        id,
        action_type: "raw",
        function: "user",
    ...
    const findItem = (items: OptimizedChatItem[], id: string) =>
      items.find((item) => item.event?.id === id);

    it("marks an assistant message followed by a tool call as collapsed narration", () => {
      const events = [
        makeUserMsg("u1", "do the thing"),
        makeAssistantMsg("a1", "Now let me read the file."),
        makeShellItem("cat file"),
      ];
      const { items } = processChatItems(events, baseOpts);
      expect(findItem(items, "a1")?.collapsedNarration).toBe(true);
    });

    it("leaves the final assistant message of a turn full brightness", () => {
      const events = [
        makeUserMsg("u1", "do the thing"),
        makeAssistantMsg("a1", "Now let me read the file."),
        makeShellItem("cat file"),
        makeAssistantMsg("a2", "Done — the file says hello."),
      ];
      const { items } = processChatItems(events, baseOpts);
      expect(findItem(items, "a1")?.collapsedNarration).toBe(true);
      expect(findItem(items, "a2")?.collapsedNarration).toBeUndefined();
    });

    it("does not collapse a message followed only by another message", () => {
      const events = [
        makeUserMsg("u1", "hi"),
        makeAssistantMsg("a1", "First paragraph."),
        makeAssistantMsg("a2", "Second paragraph."),
      ];
      const { items } = processChatItems(events, baseOpts);
      expect(findItem(items, "a1")?.collapsedNarration).toBeUndefined();
      expect(findItem(items, "a2")?.collapsedNarration).toBeUndefined();
    });

    it("does not collapse a trailing tool call from the next turn", () => {
      const events = [
        makeUserMsg("u1", "q1"),
        makeAssistantMsg("a1", "Answer to q1."),
        makeUserMsg("u2", "q2"),
        makeShellItem("ls"),
      ];
      const { items } = processChatItems(events, baseOpts);
