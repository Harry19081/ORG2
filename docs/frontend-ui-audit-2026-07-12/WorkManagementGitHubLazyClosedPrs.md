...
| `GitHubWorkItemsSurface.tsx` PR empty/loading body | Section feedback                    | keep with reason | The Open/Closed controls remain visible while the selected state loads or is empty, matching Workstation’s ability to reach Closed even when Open has no rows.              | None.                                               |
...
| `GitHubWorkItemsSurface.tsx` PR row icon           | Closed/merged status icon and color | abstract         | Reuses the shared PR status palette; open is green, merged uses primary, and closed uses danger, consistent with Workstation.                                               | Keep status mapping sourced from `shared/pr`.       |
