# Chat and Workstation header tooltip delays UI audit
...
| `src/modules/WorkStation/AppShell/AgentStationTopHeader.tsx:182`         | Workstation header controls      | keep with reason | Caption and panel controls reuse the tab-bar button primitive with an explicit chrome delay.                 | None.            |
...
| `src/modules/WorkStation/AppShell/TabBarPlusMenu/TabBarPlusMenu.tsx:115` | Workstation plus menu            | keep with reason | The standard workstation plus trigger uses the shared chrome timing and preserves menu-open suppression.     | None.            |
