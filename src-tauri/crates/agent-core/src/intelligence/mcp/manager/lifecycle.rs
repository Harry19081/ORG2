...
use crate::intelligence::mcp::config::{locate_owning_config, McpConfigFile, McpServerConfig};
...
impl McpManager {
    ...
    async fn connect_one_for_connect_all(
        ...
        if is_remote(server_config)
            && crate::intelligence::mcp::needs_auth_cache::is_cached(name).await
        {
            info!(
    ...
    pub async fn connect_server(&self, name: &str, config: &McpServerConfig) -> Result<(), String> {
        ...
        if is_remote(config) && crate::intelligence::mcp::needs_auth_cache::is_cached(name).await {
            info!(
                "[mcp:manager] Skipping '{}' connect (cached needs-auth)",
                name
            );
