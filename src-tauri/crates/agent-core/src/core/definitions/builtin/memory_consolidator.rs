//! Memory Consolidator agent — background subagent that periodically
//! merges, prunes, and reorganizes workspace memory files (auto-dream).
//!
//! Runs when enough sessions have accumulated since the last consolidation.
//! Same tool restrictions as the memory extractor.

use crate::definitions::schema::{
    AgentDefinition, AgentTier, AgentToolSelection, DelegationConfig, SessionMode, SessionModel,
};
use crate::tools::names as tool_names;

pub const MEMORY_CONSOLIDATOR_ID: &str = "builtin:memory-consolidator";

pub fn memory_consolidator() -> AgentDefinition {
    AgentDefinition {
        id: MEMORY_CONSOLIDATOR_ID.to_string(),
        name: "Memory Consolidator".to_string(),
        description: Some(
            "Background subagent that periodically consolidates and organizes workspace memory files."
                .to_string(),
        ),
        built_in: true,
        tier: AgentTier::Secondary,
        inherits_from: Some(super::BASE_AGENT_ID.to_string()),

        session_model: Some(SessionModel {
            mode: SessionMode::PerSession,
            compaction: None,
            processing_lock: false,
            max_iterations: 500,
        }),

        capabilities: None,

        // Strict allowlist — same tools as the extractor, plus moon-cycle budget.
        tools: AgentToolSelection {
            system_restrict_to_tools: Some(vec![
                tool_names::READ_FILE.to_string(),
                tool_names::CODE_SEARCH.to_string(),
                tool_names::LIST_DIR.to_string(),
                tool_names::RUN_SHELL.to_string(),
                tool_names::EDIT_FILE.to_string(),
            ]),
            ..Default::default()
        },

        // No soul_content — consolidation prompt injected as user message.
        soul_content: None,
        sovereign_prompt: false,

        temperature: Some(0.0),
        max_tokens: Some(8192),

        delegation_config: Some(DelegationConfig {
            delegatable: false,
            context_builders: Vec::new(),
        }),

        context_window: None,
        sub_agents: None,
        load_workspace_resources: None,
        load_workspace_rules: None,
        skills_config: None,
        selected_account_id: None,
        selected_model_id: None,

        icon_id: Some("moon".to_string()),

        animate: None,
        execution_mode: None,
        exec_timeout: None,
        max_tool_use_concurrency: None,
        learnings: None,

        agent_policy: None,
        reliability: None,
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_memory_consolidator_tool_set() {
        let agent = memory_consolidator();
        assert_eq!(agent.id, MEMORY_CONSOLIDATOR_ID);
        assert!(agent.built_in);

        let allowed = agent
            .tools
            .system_restrict_to_tools
            .as_ref()
            .expect("memory_consolidator must use strict system_restrict_to_tools");
        assert!(allowed.contains(&tool_names::READ_FILE.to_string()));
        assert!(allowed.contains(&tool_names::EDIT_FILE.to_string()));
        assert!(!allowed.contains(&tool_names::AGENT.to_string()));
    }

    #[test]
    fn test_memory_consolidator_max_iterations() {
        let agent = memory_consolidator();
        let session = agent.session_model.expect("should have session_model");
        assert_eq!(session.max_iterations, 500);
    }

    #[test]
    fn test_memory_consolidator_no_soul() {
        let agent = memory_consolidator();
        assert!(agent.soul_content.is_none());
    }
}
