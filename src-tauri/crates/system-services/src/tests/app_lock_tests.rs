...
use crate::app_lock::{
    ...
    ERR_PASSWORD_TOO_SHORT, ERR_THROTTLED, MAX_HINT_CHARS, MAX_PASSWORD_CHARS,
};
...
impl Fixture {
    ...
    fn with_password(&self) -> AppLockCore {
        ...
        core.set_password(None, PASSWORD, None, t0()).expect("set");
        core
    }
}
...
fn starts_disabled_and_cannot_lock_without_a_password() {
    ...
    assert_eq!(core.unlock("anything", t0()), Ok(true));
}
...
fn rejects_passwords_outside_the_length_bounds() {
    ...
    let mut core = fixture.load(t0());
    assert_eq!(
        core.set_password(None, "abc", None, t0()),
    ...
    assert_eq!(core.set_password(None, "密码锁屏", None, t0()), Ok(()));
}
...
fn lock_then_unlock_round_trip() {
    ...
    assert_eq!(core.unlock("wrong", t0()), Ok(false));
    assert!(core.status(t0()).locked);
...
fn a_locked_app_refuses_every_mutation_but_unlock() {
    ...
    assert_eq!(
        core.set_password(Some(PASSWORD), "another one", None, t0()),
...
fn changing_or_clearing_needs_the_current_password() {
    ...
    assert_eq!(
        core.set_password(None, "another one", None, t0()),
        Err(ERR_INVALID_PASSWORD.to_string())
    );
    assert_eq!(
        core.set_password(Some("wrong"), "another one", None, t0()),
        Err(ERR_INVALID_PASSWORD.to_string())
    );
    assert_eq!(
        core.clear_password("wrong", t0()),
        Err(ERR_INVALID_PASSWORD.to_string())
    );
    ...
    assert_eq!(core.unlock("another one", t0()), Ok(true));

    assert_eq!(core.clear_password("another one", t0()), Ok(()));
    let status = core.status(t0());
    assert!(!status.enabled);
    assert!(!status.locked);
...
fn wrong_guesses_are_throttled_then_released() {
    ...
    assert_eq!(core.unlock("wrong", after_first_delay), Ok(false));
...
fn throttle_delay_is_capped() {
    ...
    for _ in 0..40 {
        ...
        now += wait;
        assert_eq!(core.unlock("wrong", now), Ok(false));
    }
...
fn hint_may_not_give_the_password_away() {
    ...
    let mut core = fixture.load(t0());
    assert_eq!(
        core.set_password(None, PASSWORD, Some("it is Correct Horse, obviously"), t0()),
...
fn changing_the_password_replaces_the_hint_and_removing_it_clears_the_hint() {
    ...
    core.clear_password("another one", t0()).expect("clear");
    assert_eq!(core.status(t0()).hint, None);
