...
describe("Session launch wiring rendered UI invariants", function () {
  ...
  it("rejects an SDE launch with an empty workspace_path at the backend (pending-repo race)", async function () {
    if (!shouldRunScenario("pending-repo-backend-guard")) {
      this.skip();
      return;
    }

    try {
      // The narrow "Open Folder" race: a non personal-workspace agent is
      // launched with an empty workspace_path because the repo path had not
      // landed yet. The backend guard must fail at launch time and NOT
      // persist a workspace-less session that would die on its first message
      // with "workspace_root is empty".
      const launchResult = await invokeE2E("launchSession", {
        category: "rust_agent",
        content: `E2E pending-repo backend guard ${RUN_ID}. Should not launch.`,
        workspacePath: "",
        keySource: "own_key",
        accountId: account.id,
        model,
      ...
      if (launchResult?.ok !== false) {
        throw new Error(
          `Expected empty-workspace SDE launch to fail, got ${JSON.stringify(launchResult)}`
        );
      }
      const errorText = String(launchResult.error ?? "");
      if (!/workspace_path is required/i.test(errorText)) {
        throw new Error(
          `Empty-workspace SDE launch failed with an unexpected message: ${errorText}`
        );
      }

      // Negative (Rule 9): the failed launch must not leave an orphan session
      // row. A workspace-less session would have an empty persisted workspace.
      const sessionId = launchResult.result?.sessionId ?? launchResult.result?.session_id;
      if (sessionId) {
        await assertPersistedSessionWorkspace(sessionId, { workspaceRoot: null });
      }
    } finally {
  ...
  it("blocks an SDE launch from the rendered creator when the selected repo has no path yet", async function () {
    if (!shouldRunScenario("pending-repo-creator-guard")) {
      this.skip();
      return;
    }
    ...
    try {
      ...
      // Seed the "Open Folder in progress" state: selectedRepoId is set but the
      // matching repo row has an empty path, so effectiveSource resolves to a
      // source with repoId but empty repoPath. canLaunch only checks repoId, so
      // the send button stays ENABLED — the guard fires inside handleLaunch via
      // validateSessionConfig, surfacing a "Workspace is still loading" toast
      // and creating no session.
      unwrap(
        await invokeE2E("seedRepoIdWithoutPath", `e2e-pending-repo-${RUN_ID}`),
        "seed pending repo without path"
      );

      const activeBefore = unwrap(
        await invokeE2E("inspectChatState"),
        "inspectChatState before pending-repo guard"
      ).activeSessionId;

      await typeRenderedCreatorPrompt(
        `E2E SDE pending repo guard ${RUN_ID}. This should not launch.`,
        "pending repo SDE guard"
      );

      // The send button is clickable (canLaunch=true), so a real click drives
      // the guard exactly as a user would hit it.
      const clickState = await execJS(`
        const shell = document.querySelector('[data-testid="session-creator-chat-panel"]') ?? document;
        const button = shell.querySelector('[data-testid="chat-send-button"]');
        if (!button) return { ok: false, reason: "missing-send-button" };
        button.click();
        return { ok: true, disabled: Boolean(button.disabled) };
      `);
      if (clickState?.ok !== true) {
        throw new Error(
          `pending repo SDE guard could not click send: ${JSON.stringify(clickState)}`
        );
      }

      // Positive: the validation toast surfaces the loading guard message.
      let toastState = null;
      await browser.waitUntil(
        async () => {
          toastState = await execJS(`
            const root = document.querySelector('[data-message-root="true"]');
            const text = root?.innerText || '';
            return { text };
          `);
          return /workspace is still loading/i.test(toastState?.text ?? "");
        },
        {
          timeout: RENDER_TIMEOUT_MS,
          interval: 250,
          timeoutMsg: `pending repo SDE guard did not surface the loading toast: ${JSON.stringify(toastState)}`,
        }
      );

      // Negative (Rule 9): no session was created — active session is unchanged.
      const activeAfter = unwrap(
        await invokeE2E("inspectChatState"),
        "inspectChatState after pending-repo guard"
      ).activeSessionId;
      if (activeAfter !== activeBefore) {
        throw new Error(
          `pending repo SDE guard unexpectedly created/switched a session: before=${activeBefore} after=${activeAfter}`
        );
      }
    } finally {
      await invokeE2E("ensureRepoSelected", { repoPath: E2E_REPO_PATH });
